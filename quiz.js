(()=>{
  ...
  async function init(scriptEl){
    ...
    const nextVisibleIndex=(from)=>{
      let i=from; while(i<state.questions.length && !passesShowIf(state.flags,state.questions[i])) i++;
    ...
    function renderQuestion(q){
      ...
      if(multi){
        ...
        on(nextBtn,'click',()=>{
          ...
          // apply all selected in config order
    ...
    function showResult(forcedId){
      ...
      if(rn.text){ const tx=el('div','quiz-result-text'); tx.textContent=rn.text; res.appendChild(tx); }

      const ctas=el('div','quiz-ctas');
      const a=el('a','quiz-cta'); a.href=rn.cta_url||resOpt.cta||cfg.cta_url||'#'; a.textContent=rn.cta_label||resOpt.cta_label||cfg.cta_label||'Continue'; ctas.appendChild(a);
      if(cfg.cta2_url&&cfg.cta2_label){ const a2=el('a','quiz-cta-secondary'); a2.href=cfg.cta2_url; a2.textContent=cfg.cta2_label; ctas.appendChild(a2); }
      const re=el('button','quiz-restart'); re.textContent='Restart'; on(re,'click',()=>{ GA('quiz_restart',{quiz_id:cfg.id}); location.reload(); }); ctas.appendChild(re);
    ...
    function render(){
      ...
      const container=el('div','quiz'); header(container);
      if(q){ container.appendChild(renderQuestion(q)); } else { showResult(); return; }
